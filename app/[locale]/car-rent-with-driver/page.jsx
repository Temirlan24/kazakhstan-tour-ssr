...
const OG_IMAGE = '/assets/cars/camry_80/camry_80_1.webp';

export async function generateMetadata({ params }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'seo.carRentWithDriver' });
...
export default async function CarWithDriverPage({ params }) {
  const { locale } = await params;
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: t('carRentWithDriver.breadcrumb'), item: pageUrl },
    ],
  };
  ...
  const serviceSchema = {
    '@context': 'https://schema.org',
    '@type': 'Service',
    name: t('carRentWithDriver.schemaName'),
    description: t('carRentWithDriver.schemaDescription'),
